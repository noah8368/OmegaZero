...
Run this once before unc_research/scripts/train_unc_head.py. It chains the two
...
generate_unc_plots.py's auto-encode paths use, so you do NOT strictly need to run this by
...
Usage:
    python3 unc_research/scripts/prepare_unc_data.py                          # nnue/data_uncertainty
    python3 unc_research/scripts/prepare_unc_data.py /path/to/unc_data        # custom data dir
    python3 unc_research/scripts/prepare_unc_data.py --skip-combine           # re-encode only
"""
...
def encode_uncertainty(input_path, output_path):
    ...
    (unc_research/scripts/train_unc_head.py) and generate_unc_plots.py can (re)encode on
    ...
    # Encode into a temp file and atomically rename on success. A pre-sized
    # mode="w+" memmap is zero-filled up front, so a killed/crashed encode would
    # otherwise leave a FULL-SIZE, mostly-zero .bin at the final path that a
    # mtime-only staleness check happily accepts (this bit us: the 2026-09-05
    # unc-head run trained on a .bin that was 98% zero-filled). With the rename,
    # the final path only ever appears once the encode has fully completed.
    tmp_path = output_path.with_name(output_path.name + ".tmp")
    idx = 0
    skipped = 0
    mismatched_u = 0
    try:
        data = np.memmap(tmp_path, dtype=UNC_RECORD_DTYPE, mode="w+",
                         shape=(num_lines,))
        ...
        data.flush()
        del data  # release the memmap handle before truncate + rename

        if idx < num_lines:
            with open(tmp_path, "r+b") as f:
                f.truncate(idx * UNC_RECORD_DTYPE.itemsize)

        os.replace(tmp_path, output_path)  # atomic: final path == complete encode
    finally:
        # If os.replace ran, tmp_path is gone; otherwise (failure/kill) drop the
        # partial temp so it can never be mistaken for a finished encode.
        if tmp_path.exists():
            tmp_path.unlink()
