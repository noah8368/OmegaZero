...
# `result` is stored WHITE POV (datagen.cc ResultToStr) but v/v* are STM POV, so
# convert to STM POV before grouping the residual by it: flip when Black to move.
is_white = np.array([s == "w" for s in fens_stm], dtype=bool)
...
# 6: residual by game result (result converted to STM POV to match v/v*)
labels = [("loss", 0.0), ("draw", 0.5), ("win", 1.0)]
data = [r[result_stm == val] for _, val in labels]
