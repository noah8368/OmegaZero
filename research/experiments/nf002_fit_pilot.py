...
-> mdn_t fit -> PIT/coverage/qMAE), and that conditioning beats the unconditional
floor -- but a coherent calibration result requires regenerating labels with the
NNUE eval. See research/experiments/NF-002.md.
...
Usage:
  .venv/bin/python research/experiments/nf002_fit_pilot.py \
      --trunk nnue/model/2026-06-07_00-11-38_61d0444_6.0M_pos/best.bin \
      --train nnue/data_uncertainty/combined/training_data.bin \
      --val   nnue/data_uncertainty/combined/validation_data.bin
"""
...
from preprocess_uncertainty import UNC_RECORD_DTYPE  # noqa: E402
...
def main():
    ...
    xtr, utr = load_split(args.train, "train")
    xva, uva = load_split(args.val, "val")
    ...
    cond_val = train(cond, xtr_t, ytr_t, xva_t, yva_t, args.epochs, args.bs, args.lr, "cond")
    ...
    unc_val = train(uncond, ztr, ytr_t, zva, yva_t, args.epochs, args.bs, args.lr, "uncond")
    unc_cal = calibration(uncond, zva, yva_t, u_mean, u_std)

    print("\n" + "=" * 62)
    print("NF-002 FIT PILOT RESULTS  (harness check -- labels=HCE, x=NNUE)")
    print("=" * 62)
    ...
    print("near nominal + small PIT KS => the head is calibrated on this (incoherent)")
    print("data. A real read needs NNUE-eval labels (regen datagen with nnue.bin).")
